...
public class Caxeiro_Viajante {
        ...
        public Caxeiro_Viajante(AED2_EdgeWeightedDigraph G,int s,double maxkm) {
           ...
           int v=s;     // run from each vertex to find
                if (!marked[v]) prim(G, v,maxkm);      // minimum spanning forest
        ...
        private void prim(AED2_EdgeWeightedDigraph G, int s,double maxkm) {
            distTo[s] = 0.0;
        ...
        // scan vertex v
        private void scan(AED2_EdgeWeightedDigraph G, int v,double maxkm) {
            marked[v] = true;
            for (Edge_Project e : G.adj(v)) {
                int w = e.to();
                double weight=weight()+e.weight();
                if(weight>=maxkm)return;
